...
def test_simple_select():
    ...
    query = SelectStatement(
        select_list=[
            ...
            SelectItem(b.func("COUNT", b.star()), Identifier("order_count")),
        ],
    ...
    custom_options = SerializerOptions(
        uppercase_keywords=False, quote_identifiers=False, format_style="expanded"
    )
...
def test_complex_merge():
    ...
    merge = MergeStatement(
        ...
        when_clauses=[
            ...
            MergeWhenClause(
                match_type="NOT MATCHED BY TARGET", action=MergeInsert(row=True)
            ),
            MergeWhenClause(
        ...
        select_list=[
            ...
            SelectItem(
                WindowFunction(
                    ...
                    window=WindowSpecification(
                        partition_by=[b.col("customer_id")],
                        order_by=[OrderByItem(b.col("order_date"), OrderDirection.ASC)],
                    ),
                ),
                Identifier("running_total"),
            ),
        ],
        from_clause=FromClause([TableReference(TableName(table=Identifier("orders")))]),
    )
