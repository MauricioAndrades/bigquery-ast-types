...
def demo_ast_manipulation():
    ...
    query.find(exp.Column).forEach(lambda node, i: 
    ...
    def transform_string_ids(node):
        if node.name in string_id_cols:
            ...
            return exp.func("NULLIF", 
                exp.func("TRIM", node.node), 
                exp.Literal.string("")
            )
        return node.node
    
    query.find(exp.Column, 
        predicate=lambda n: n.name in string_id_cols
    ).replaceWith(transform_string_ids)
...
def demo_dedup_patterns():
    ...
    all_columns = [
        'retailer_id', 'order_id', 'product_id', 'visitor_id',
        'order_ts', 'session_id', 'quantity', 'price',
        'child_product_id', 'hashed_pii'
    ]
