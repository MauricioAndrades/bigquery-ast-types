...
from typing import Any, Callable, Optional, List, Union, Iterator as PyIterator, TypeVar, Generic
...
class Iterator(Generic[T]):
    ...
    def __init__(self, node: exp.Expression, node_type: Optional[type[T]] = None, predicate: Optional[Callable] = None):
        self.node = node
    ...
    def filter(self, fn: Callable[['JSQLNode'], bool]) -> 'Iterator':
        ...
        combined_predicate = lambda n: (self.predicate(n) if self.predicate else True) and fn(n)
        return Iterator(self.node, self.node_type, combined_predicate)

    def replaceWith(self, fn: Union[Callable[['JSQLNode'], exp.Expression], exp.Expression]) -> 'Iterator':
...
class JSQLNode:
    ...
    def find(self, node_type: type[T], predicate: Optional[Callable] = None) -> Iterator[T]:
...
class j:
    ...
    @staticmethod
    def Iterator(node: Union[JSQLNode, exp.Expression], 
    ...
    @staticmethod
    def Table(name: str, db: Optional[str] = None, catalog: Optional[str] = None) -> JSQLNode:
    ...
    def Case(*whens, default: Optional[exp.Expression] = None) -> JSQLNode:
        ...
        for when in whens:
            if isinstance(when, tuple) and len(when) == 2:
                ...
                case_exp.args["ifs"].append(
                    exp.If(this=condition, true=result)
                )
    ...
    def Cast(expr: exp.Expression, to: str, safe: bool = False) -> JSQLNode:
        ...
        if safe:
            return JSQLNode(exp.SafeCast(this=expr, to=exp.DataType.build(to)))
    ...
    def CTE(name: str, query: Union[str, exp.Expression]) -> JSQLNode:
        ...
        if isinstance(query, str):
            ...
            query = query.node
        return JSQLNode(exp.CTE(this=query, alias=name))

    @staticmethod
    def Window(func: exp.Expression, 
               partition_by: Optional[List[exp.Expression]] = None,
               order_by: Optional[List[exp.Expression]] = None) -> JSQLNode:
...
def standardize_numeric(col: exp.Expression) -> exp.Expression:
    ...
    return exp.func(
        "COALESCE",
        exp.SafeCast(this=col, to=exp.DataType.build("NUMERIC")),
...
def deep_copy_transform(node: exp.Expression, 
                       transform_fn: Callable[[exp.Expression], Optional[exp.Expression]]) -> exp.Expression:
...
def inject_cte(ast: exp.Expression, cte_name: str, cte_query: exp.Expression) -> exp.Expression:
