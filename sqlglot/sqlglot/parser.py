...
def binary_range_parser(expr_type: t.Type[exp.Expression], reverse_args: bool = False) -> t.Callable[[Parser, t.Optional[exp.Expression]], t.Optional[exp.Expression]]:
    def _parse_binary_range(self: Parser, this: t.Optional[exp.Expression]) -> t.Optional[exp.Expression]:
...
def build_array_constructor(exp_class: t.Type[E], args: t.List, bracket_kind: TokenType, dialect: Dialect) -> exp.Expression:
...
def build_convert_timezone(args: t.List, default_source_tz: t.Optional[str] = None) -> t.Union[exp.ConvertTimezone, exp.Anonymous]:
    if len(args) == 2:
        source_tz = exp.Literal.string(default_source_tz) if default_source_tz else None
        return exp.ConvertTimezone(source_tz=source_tz, target_tz=seq_get(args, 0), timestamp=seq_get(args, 1))
...
def build_coalesce(args: t.List, is_nvl: t.Optional[bool] = None, is_null: t.Optional[bool] = None) -> exp.Coalesce:
...
class Parser(metaclass=_Parser):
    ...
    FUNCTIONS: t.Dict[str, t.Callable] = {
        ...
        **dict.fromkeys(("COALESCE", "IFNULL", "NVL"), build_coalesce),
        ...
        "ARRAYAGG": lambda args, dialect: exp.ArrayAgg(this=seq_get(args, 0), nulls_excluded=dialect.ARRAY_AGG_INCLUDES_NULLS is None or None),
        "ARRAY_AGG": lambda args, dialect: exp.ArrayAgg(this=seq_get(args, 0), nulls_excluded=dialect.ARRAY_AGG_INCLUDES_NULLS is None or None),
        ...
        "SCOPE_RESOLUTION": lambda args: exp.ScopeResolution(expression=seq_get(args, 0)) if len(args) != 2 else exp.ScopeResolution(this=seq_get(args, 0), expression=seq_get(args, 1)),
    ...
    COLUMN_OPERATORS = {
        ...
        TokenType.DCOLON: lambda self, this, to: self.build_cast(strict=self.STRICT_CAST, this=this, to=to),
    ...
    STRING_PARSERS = {
        TokenType.HEREDOC_STRING: lambda self, token: self.expression(exp.RawString, this=token.text),
        TokenType.NATIONAL_STRING: lambda self, token: self.expression(exp.National, this=token.text),
    ...
    PLACEHOLDER_PARSERS = {
        ...
        TokenType.COLON: lambda self: (self.expression(exp.Placeholder, this=self._prev.text) if self._match_set(self.COLON_PLACEHOLDER_TOKENS) else None),
    }
    ...
    PROPERTY_PARSERS: t.Dict[str, t.Callable] = {
        "ALLOWED_VALUES": lambda self: self.expression(exp.AllowedValuesProperty, expressions=self._parse_csv(self._parse_primary)),
        ...
        "BACKUP": lambda self: self.expression(exp.BackupProperty, this=self._parse_var(any_token=True)),
        ...
        "COLLATE": lambda self, **kwargs: self._parse_property_assignment(exp.CollateProperty, **kwargs),
        ...
        "DETERMINISTIC": lambda self: self.expression(exp.StabilityProperty, this=exp.Literal.string("IMMUTABLE")),
        ...
        "ENVIRONMENT": lambda self: self.expression(exp.EnviromentProperty, expressions=self._parse_wrapped_csv(self._parse_assignment)),
        ...
        "IMMUTABLE": lambda self: self.expression(exp.StabilityProperty, this=exp.Literal.string("IMMUTABLE")),
        "INHERITS": lambda self: self.expression(exp.InheritsProperty, expressions=self._parse_wrapped_csv(self._parse_table)),
        ...
        "SAMPLE": lambda self: self.expression(exp.SampleProperty, this=self._match_text_seq("BY") and self._parse_bitwise()),
        ...
        "STABLE": lambda self: self.expression(exp.StabilityProperty, this=exp.Literal.string("STABLE")),
        ...
        "TRANSFORM": lambda self: self.expression(exp.TransformModelProperty, expressions=self._parse_wrapped_csv(self._parse_expression)),
    ...
    CONSTRAINT_PARSERS = {
        ...
        "CHARACTER SET": lambda self: self.expression(exp.CharacterSetColumnConstraint, this=self._parse_var_or_string()),
        ...
        "CLUSTERED": lambda self: self.expression(exp.ClusteredColumnConstraint, this=self._parse_wrapped_csv(self._parse_ordered)),
        "NONCLUSTERED": lambda self: self.expression(exp.NonClusteredColumnConstraint, this=self._parse_wrapped_csv(self._parse_ordered)),
        "DEFAULT": lambda self: self.expression(exp.DefaultColumnConstraint, this=self._parse_bitwise()),
        "ENCODE": lambda self: self.expression(exp.EncodeColumnConstraint, this=self._parse_var()),
        "EPHEMERAL": lambda self: self.expression(exp.EphemeralColumnConstraint, this=self._parse_bitwise()),
        "EXCLUDE": lambda self: self.expression(exp.ExcludeColumnConstraint, this=self._parse_index_params()),
        "FOREIGN KEY": lambda self: self._parse_foreign_key(),
        "FORMAT": lambda self: self.expression(exp.DateFormatColumnConstraint, this=self._parse_var_or_string()),
        ...
        "ON": lambda self: (self._match(TokenType.UPDATE) and self.expression(exp.OnUpdateColumnConstraint, this=self._parse_function())) or self.expression(exp.OnProperty, this=self._parse_id_var()),
        ...
        "WITH": lambda self: self.expression(exp.Properties, expressions=self._parse_wrapped_properties()),
    ...
    def _parse_partitioned_by_bucket_or_truncate(self) -> t.Optional[exp.Expression]:
        ...
        klass = exp.PartitionedByBucket if self._prev.text.upper() == "BUCKET" else exp.PartitionByTruncate
    ...
    ALTER_PARSERS = {
        ...
        "SWAP": lambda self: self.expression(exp.SwapTable, this=self._match(TokenType.WITH) and self._parse_table(schema=True)),
    }
    ...
    CONFLICT_ACTIONS: OPTIONS_TYPE = dict.fromkeys(("ABORT", "FAIL", "IGNORE", "REPLACE", "ROLLBACK", "UPDATE"), tuple())
    ...
    USABLES: OPTIONS_TYPE = dict.fromkeys(("ROLE", "WAREHOUSE", "DATABASE", "SCHEMA", "CATALOG"), tuple())
    ...
    def expression(self, exp_class: t.Type[E], comments: t.Optional[t.List[str]] = None, **kwargs) -> E:
        """
    ...
    def _parse_drop(self, exists: bool = False) -> exp.Drop | exp.Command:
        ...
        if kind == "COLUMN":
            ...
            this = self._parse_table_parts(schema=True, is_db_reference=self._prev.token_type == TokenType.SCHEMA)
    ...
    def _parse_exists(self, not_: bool = False) -> t.Optional[bool]:
        return self._match_text_seq("IF") and (not not_ or self._match(TokenType.NOT)) and self._match(TokenType.EXISTS)

    def _parse_create(self) -> exp.Create | exp.Command:
        # Note: this can't be None because we've matched a statement parser
        start = self._prev

        replace = start.token_type == TokenType.REPLACE or self._match_pair(TokenType.OR, TokenType.REPLACE) or self._match_pair(TokenType.OR, TokenType.ALTER)
        ...
        if self._match_text_seq("CLUSTERED", "COLUMNSTORE"):
            clustered = True
        elif self._match_text_seq("NONCLUSTERED", "COLUMNSTORE") or self._match_text_seq("COLUMNSTORE"):
            clustered = False
        else:
            clustered = None
        ...
        if create_token.token_type in (TokenType.FUNCTION, TokenType.PROCEDURE):
            ...
            table_parts = self._parse_table_parts(schema=True, is_db_reference=create_token.token_type == TokenType.SCHEMA)
    ...
    def _parse_property_before(self) -> t.Optional[exp.Expression]:
        ...
        kwargs = {
            ...
            "local": (self._match_text_seq("LOCAL") and "LOCAL") or (self._match_text_seq("NOT", "LOCAL") and "NOT LOCAL"),
    ...
    def _parse_fallback(self, no: bool = False) -> exp.FallbackProperty:
        return self.expression(exp.FallbackProperty, no=no, protection=self._match_text_seq("PROTECTION"))
    ...
    def _parse_settings_property(self) -> exp.SettingsProperty:
        return self.expression(exp.SettingsProperty, expressions=self._parse_csv(self._parse_assignment))
    ...
    def _parse_system_versioning_property(self, with_: bool = False) -> exp.WithSystemVersioningProperty:
        self._match(TokenType.EQ)
    ...
    def _parse_cluster(self, wrapped: bool = False) -> exp.Cluster:
        return self.expression(
            exp.Cluster,
            expressions=(self._parse_wrapped_csv(self._parse_ordered) if wrapped else self._parse_csv(self._parse_ordered)),
        )
    ...
    def _parse_freespace(self) -> exp.FreespaceProperty:
        self._match(TokenType.EQ)
        return self.expression(exp.FreespaceProperty, this=self._parse_number(), percent=self._match(TokenType.PERCENT))
    ...
    def _parse_sortkey(self, compound: bool = False) -> exp.SortKeyProperty:
        return self.expression(exp.SortKeyProperty, this=self._parse_wrapped_id_vars(), compound=compound)

    def _parse_character_set(self, default: bool = False) -> exp.CharacterSetProperty:
        self._match(TokenType.EQ)
        return self.expression(exp.CharacterSetProperty, this=self._parse_var_or_string(), default=default)

    def _parse_remote_with_connection(self) -> exp.RemoteWithConnectionModelProperty:
        self._match_text_seq("WITH", "CONNECTION")
        return self.expression(exp.RemoteWithConnectionModelProperty, this=self._parse_table_parts())
    ...
    def _parse_wrapped_select(self, table: bool = False) -> t.Optional[exp.Expression]:
        if self._match_set((TokenType.PIVOT, TokenType.UNPIVOT)):
            this: t.Optional[exp.Expression] = self._parse_simplified_pivot(is_unpivot=self._prev.token_type == TokenType.UNPIVOT)
        elif self._match(TokenType.FROM):
            ...
            this = self._parse_table(consume_pipe=True) if table else self._parse_select(nested=True, parse_set_operation=False)
    ...
    def _parse_select(
        ...
        if consume_pipe and self._match(TokenType.PIPE_GT, advance=False) and isinstance(query, exp.Query):
    ...
    def _parse_select_query(
        ...
        from_ = self._parse_from(consume_pipe=True) if self._match(TokenType.FROM, advance=False) else None
    ...
    def _parse_lateral(self) -> t.Optional[exp.Lateral]:
        ...
        if not this:
            this = self._parse_unnest() or self._parse_function() or self._parse_id_var(any_token=False)
        ...
        if view:
            ...
            table_alias: t.Optional[exp.TableAlias] = self.expression(exp.TableAlias, this=table, columns=columns)
    ...
    def _parse_join(self, skip_join_token: bool = False, parse_bracket: bool = False) -> t.Optional[exp.Join]:
        ...
        if kind and kind.token_type == TokenType.ARRAY and self._match(TokenType.COMMA):
            kwargs["expressions"] = self._parse_csv(lambda: self._parse_table(parse_bracket=parse_bracket))
        ...
        elif not (outer_apply or cross_apply) and not isinstance(kwargs["this"], exp.Unnest) and not (kind and kind.token_type in (TokenType.CROSS, TokenType.ARRAY)):
            index = self._index
    ...
    def _parse_index(self, index: t.Optional[exp.Expression] = None, anonymous: bool = False) -> t.Optional[exp.Index]:
    ...
    def _parse_table_hints(self) -> t.Optional[t.List[exp.Expression]]:
        ...
        if self._match_pair(TokenType.WITH, TokenType.L_PAREN):
            ...
            hints.append(
                self.expression(
                    exp.WithTableHint,
                    expressions=self._parse_csv(lambda: self._parse_function() or self._parse_var(any_token=True)),
                )
            )
            self._match_r_paren()
        else:
    ...
    def _parse_table_part(self, schema: bool = False) -> t.Optional[exp.Expression]:
        return (not schema and self._parse_function(optional_parens=False)) or self._parse_id_var(any_token=False) or self._parse_string_as_identifier() or self._parse_placeholder()
    ...
    def _parse_table_parts(self, schema: bool = False, is_db_reference: bool = False, wildcard: bool = False) -> exp.Table:
        ...
        while self._match(TokenType.DOT):
            if catalog:
                ...
                table = self.expression(exp.Dot, this=table, expression=self._parse_table_part(schema=schema))
            else:
                catalog = db
                db = table
        ...
        if wildcard and self._is_connected() and (isinstance(table, exp.Identifier) or not table) and self._match(TokenType.STAR):
    ...
    def _parse_table(
        ...
        rows_from = self._match_text_seq("ROWS", "FROM") and self._parse_wrapped_csv(self._parse_table)
        ...
        this = t.cast(
            exp.Expression,
            bracket or rows_from or self._parse_bracket(self._parse_table_parts(schema=schema, is_db_reference=is_db_reference)),
        )
        ...
        if isinstance(this, exp.Table) and self._match_text_seq("AT"):
            return self.expression(exp.AtIndex, this=this.to_column(copy=False), expression=self._parse_id_var())
    ...
    def _parse_version(self) -> t.Optional[exp.Version]:
        ...
        if self._match_set((TokenType.FROM, TokenType.BETWEEN)):
            ...
            expression: t.Optional[exp.Expression] = self.expression(exp.Tuple, expressions=[start, end])
        elif self._match_text_seq("CONTAINED", "IN"):
            kind = "CONTAINED IN"
            expression = self.expression(exp.Tuple, expressions=self._parse_wrapped_csv(self._parse_bitwise))
    ...
    def _parse_historical_data(self) -> t.Optional[exp.HistoricalData]:
        ...
        if self._match_texts(self.HISTORICAL_DATA_PREFIX):
            ...
            if expression:
                self._match_r_paren()
                historical_data = self.expression(exp.HistoricalData, this=this, kind=kind, expression=expression)
            else:
                self._retreat(index)
    ...
    def _parse_pivot(self) -> t.Optional[exp.Pivot]:
        ...
        default_on_null = self._match_text_seq("DEFAULT", "ON", "NULL") and self._parse_wrapped(self._parse_bitwise)
        ...
        if not unpivot:
            ...
            for pivot_field in pivot.fields:
                ...
                all_fields.append([fld.sql() if self.IDENTIFY_PIVOT_STRINGS else fld.alias_or_name for fld in pivot_field_expressions])
    ...
    def _parse_where(self, skip_where_token: bool = False) -> t.Optional[exp.Where]:
        ...
        return self.expression(exp.Where, comments=self._prev_comments, this=self._parse_assignment())
    ...
    def _parse_group(self, skip_group_by_token: bool = False) -> t.Optional[exp.Group]:
        ...
        while True:
            ...
            elements["expressions"].extend(self._parse_csv(lambda: None if self._match_set((TokenType.CUBE, TokenType.ROLLUP), advance=False) else self._parse_assignment()))

            before_with_index = self._index
            with_prefix = self._match(TokenType.WITH)

            if self._match(TokenType.ROLLUP):
                elements["rollup"].append(self._parse_cube_or_rollup(exp.Rollup, with_prefix=with_prefix))
            elif self._match(TokenType.CUBE):
                elements["cube"].append(self._parse_cube_or_rollup(exp.Cube, with_prefix=with_prefix))
    ...
    def _parse_cube_or_rollup(self, kind: t.Type[E], with_prefix: bool = False) -> E:
        return self.expression(kind, expressions=[] if with_prefix else self._parse_wrapped_csv(self._parse_column))
    ...
    def _parse_having(self, skip_having_token: bool = False) -> t.Optional[exp.Having]:
        if not skip_having_token and not self._match(TokenType.HAVING):
            return None
        return self.expression(exp.Having, comments=self._prev_comments, this=self._parse_assignment())
    ...
    def _parse_order(self, this: t.Optional[exp.Expression] = None, skip_order_token: bool = False) -> t.Optional[exp.Expression]:
        siblings = None
    ...
    def _parse_ordered(self, parse_method: t.Optional[t.Callable] = None) -> t.Optional[exp.Ordered]:
        ...
        if not explicitly_null_ordered and ((not desc and self.dialect.NULL_ORDERING == "nulls_are_small") or (desc and self.dialect.NULL_ORDERING != "nulls_are_small")) and self.dialect.NULL_ORDERING != "nulls_are_last":
            nulls_first = True
        ...
        return self.expression(exp.Ordered, this=this, desc=desc, nulls_first=nulls_first, with_fill=with_fill)
    ...
    def _parse_offset(self, this: t.Optional[exp.Expression] = None) -> t.Optional[exp.Expression]:
        ...
        return self.expression(exp.Offset, this=this, expression=count, expressions=self._parse_limit_by())
    ...
    def _can_parse_limit_or_offset(self) -> bool:
        ...
        index = self._index
        result = bool(self._try_parse(self._parse_limit, retreat=True) or self._try_parse(self._parse_offset, retreat=True))
        self._retreat(index)
        return result
    ...
    def _parse_locks(self) -> t.List[exp.Lock]:
        ...
        while True:
            ...
            locks.append(self.expression(exp.Lock, update=update, expressions=expressions, wait=wait, key=key))
    ...
    def _parse_assignment(self) -> t.Optional[exp.Expression]:
        ...
        if not this and self._next and self._next.token_type in self.ASSIGNMENT:
            ...
            this = exp.column(t.cast(str, self._advance_any(ignore_reserved=True) and self._prev.text))
    ...
    def _parse_is(self, this: t.Optional[exp.Expression]) -> t.Optional[exp.Expression]:
        ...
        if self._match(TokenType.JSON):
            ...
            expression: t.Optional[exp.Expression] = self.expression(exp.JSON, **{"this": kind, "with": _with, "unique": unique})
        else:
    ...
    def _parse_interval(self, match_interval: bool = True) -> t.Optional[exp.Add | exp.Interval]:
        ...
        if not this or (isinstance(this, exp.Column) and not this.table and not this.this.quoted and this.name.upper() == "IS"):
            self._retreat(index)
            return None

        unit = self._parse_function() or (not self._match(TokenType.ALIAS, advance=False) and self._parse_var(any_token=True, upper=True))
        ...
        if self.INTERVAL_SPANS and self._match_text_seq("TO"):
            unit = self.expression(exp.IntervalSpan, this=unit, expression=self._parse_var(any_token=True, upper=True))
        ...
        if self._match_set((TokenType.STRING, TokenType.NUMBER), advance=False):
            return self.expression(exp.Add, this=interval, expression=self._parse_interval(match_interval=False))
    ...
    def _parse_bitwise(self) -> t.Optional[exp.Expression]:
        ...
        while True:
            if self._match_set(self.BITWISE):
                ...
                this = self.expression(exp.Coalesce, this=this, expressions=ensure_list(self._parse_term()))
    ...
    def _parse_type(self, parse_interval: bool = True, fallback_to_identifier: bool = False) -> t.Optional[exp.Expression]:
        ...
        if data_type:
            ...
            if isinstance(this, exp.Literal):
                ...
                if self.ZONE_AWARE_TIMESTAMP_CONSTRUCTOR and data_type.is_type(exp.DataType.Type.TIMESTAMP) and TIME_ZONE_RE.search(literal):
    ...
    def _parse_types(self, check_func: bool = False, schema: bool = False, allow_identifiers: bool = True) -> t.Optional[exp.Expression]:
        ...
        if not self._match_set(self.TYPE_TOKENS):
            identifier = allow_identifiers and self._parse_id_var(any_token=False, tokens=(TokenType.VAR,))
        ...
        if type_token == TokenType.MAP and self._match(TokenType.L_BRACKET):
            key_type = self._parse_types(check_func=check_func, schema=schema, allow_identifiers=allow_identifiers)
            if not self._match(TokenType.FARROW):
                self._retreat(index)
                return None

            value_type = self._parse_types(check_func=check_func, schema=schema, allow_identifiers=allow_identifiers)
        ...
        if self._match(TokenType.L_PAREN):
            if is_struct:
                ...
                expressions = self._parse_csv(lambda: self._parse_types(check_func=check_func, schema=schema, allow_identifiers=allow_identifiers))
                ...
                func_or_ident = self._parse_function(anonymous=True) or self._parse_id_var(any_token=False, tokens=(TokenType.VAR, TokenType.ANY))
        ...
        if type_token in self.TIMESTAMPS:
            if self._match_text_seq("WITH", "TIME", "ZONE"):
                maybe_func = False
                tz_type = exp.DataType.Type.TIMETZ if type_token in self.TIMES else exp.DataType.Type.TIMESTAMPTZ
        ...
        while self._curr:
            ...
            if (not matched_l_bracket and not matched_array) or (datatype_token == TokenType.ARRAY and self._match(TokenType.R_BRACKET)):
            ...
            if values and not schema and (not self.dialect.SUPPORTS_FIXED_SIZE_ARRAYS or datatype_token == TokenType.ARRAY):
            ...
            this = exp.DataType(this=exp.DataType.Type.ARRAY, expressions=[this], values=values, nested=True)
    ...
    def _parse_struct_types(self, type_required: bool = False) -> t.Optional[exp.Expression]:
        ...
        if self._curr and self._next and self._curr.token_type in self.TYPE_TOKENS and self._next.token_type in self.TYPE_TOKENS:
            ...
            this = self._parse_type(parse_interval=False, fallback_to_identifier=True) or self._parse_id_var()

        self._match(TokenType.COLON)

        if type_required and not isinstance(this, exp.DataType) and not self._match_set(self.TYPE_TOKENS, advance=False):
            self._retreat(index)
    ...
    def _parse_colon_as_variant_extract(self, this: t.Optional[exp.Expression]) -> t.Optional[exp.Expression]:
        casts = []
        json_path = []
        escape = None
        ...
        while self._match(TokenType.COLON):
            ...
            path = self._parse_column_ops(self._parse_field(any_token=True, tokens=(TokenType.SELECT,)))
            ...
            if casts:
                dcolon_offset = next(i for i, t in enumerate(self._tokens[start_index:]) if t.token_type == TokenType.DCOLON)
    ...
    def _parse_column_ops(self, this: t.Optional[exp.Expression]) -> t.Optional[exp.Expression]:
        ...
        while self._match_set(self.COLUMN_OPERATORS):
            ...
            if isinstance(field, (exp.Func, exp.Window)) and this:
                this = this.transform(lambda n: n.to_dot(include_dots=False) if isinstance(n, exp.Column) else n)
    ...
    def _parse_field(
        ...
        if anonymous_func:
            field = self._parse_function(anonymous=anonymous_func, any_token=any_token) or self._parse_primary()
        else:
            field = self._parse_primary() or self._parse_function(anonymous=anonymous_func, any_token=any_token)
    ...
    def _parse_function(
        ...
        fn_syntax = False
        if self._match(TokenType.L_BRACE, advance=False) and self._next and self._next.text.upper() == "FN":
            self._advance(2)
            fn_syntax = True
    ...
    def _parse_function_call(
        ...
        if parser and not anonymous:
            ...
            if known_function and post_func_comments:
                ...
                if any(comment.lstrip().startswith(exp.SQLGLOT_ANONYMOUS) for comment in post_func_comments):
    ...
    def _kv_to_prop_eq(self, expressions: t.List[exp.Expression], parse_map: bool = False) -> t.List[exp.Expression]:
        transformed = []
    ...
    def _parse_user_defined_function(self, kind: t.Optional[TokenType] = None) -> t.Optional[exp.Expression]:
        ...
        self._match_r_paren()
        return self.expression(exp.UserDefinedFunction, this=this, expressions=expressions, wrapped=True)
    ...
    def _parse_lambda(self, alias: bool = False) -> t.Optional[exp.Expression]:
        ...
        if self._match(TokenType.L_PAREN):
            expressions = t.cast(t.List[t.Optional[exp.Expression]], self._parse_csv(self._parse_lambda_arg))
        ...
        if self._match(TokenType.DISTINCT):
            this = self.expression(exp.Distinct, expressions=self._parse_csv(self._parse_assignment))
        else:
            this = self._parse_select_or_expression(alias=alias)

        return self._parse_limit(self._parse_order(self._parse_having_max(self._parse_respect_or_ignore_nulls(this))))
    ...
    def _parse_column_def(self, this: t.Optional[exp.Expression], computed_column: bool = True) -> t.Optional[exp.Expression]:
        ...
        elif kind and self._match(TokenType.ALIAS, advance=False) and (not self.WRAPPED_TRANSFORM_COLUMN_CONSTRAINT or (self._next and self._next.token_type == TokenType.L_PAREN)):
            self._advance()
            constraints.append(
                self.expression(
                    ...
                    kind=exp.ComputedColumnConstraint(
                        this=self._parse_disjunction(),
                        persisted=self._match_texts(("STORED", "VIRTUAL")) and self._prev.text.upper() == "STORED",
                    ),
                )
            )
    ...
    def _parse_auto_increment(
        ...
        if start and increment:
            return exp.GeneratedAsIdentityColumnConstraint(start=start, increment=increment, this=False, order=order)
    ...
    def _parse_generated_as_identity(
        ...
        if self._match_text_seq("BY", "DEFAULT"):
            ...
            this = self.expression(exp.GeneratedAsIdentityColumnConstraint, this=False, on_null=on_null)
        else:
    ...
    def _parse_column_constraint(self) -> t.Optional[exp.Expression]:
        ...
        procedure_option_follows = self._match(TokenType.WITH, advance=False) and self._next and self._next.text.upper() in self.PROCEDURE_OPTIONS
    ...
    def _parse_key_constraint_options(self) -> t.List[str]:
        ...
        while True:
            ...
            if self._match(TokenType.ON):
                ...
                var = self._parse_var_from_options(self.KEY_CONSTRAINT_OPTIONS, raise_unmatched=False)
                if not var:
                    break
    ...
    def _parse_foreign_key(self) -> exp.ForeignKey:
        expressions = self._parse_wrapped_id_vars() if not self._match(TokenType.REFERENCES, advance=False) else None
    ...
    def _parse_primary_key(self, wrapped_optional: bool = False, in_props: bool = False) -> exp.PrimaryKeyColumnConstraint | exp.PrimaryKey:
        desc = self._match_set((TokenType.ASC, TokenType.DESC)) and self._prev.token_type == TokenType.DESC
        ...
        expressions = self._parse_wrapped_csv(self._parse_primary_key_part, optional=wrapped_optional)
    ...
    def _parse_bracket(self, this: t.Optional[exp.Expression] = None) -> t.Optional[exp.Expression]:
        ...
        if bracket_kind == TokenType.L_BRACE and self._curr and self._curr.token_type == TokenType.VAR and self._curr.text.lower() in self.ODBC_DATETIME_LITERALS:
            return self._parse_odbc_datetime_literal()

        expressions = self._parse_csv(lambda: self._parse_bracket_key_value(is_map=bracket_kind == TokenType.L_BRACE))
        ...
        if bracket_kind == TokenType.L_BRACE:
            ...
            this = build_array_constructor(exp.Array, args=expressions, bracket_kind=bracket_kind, dialect=self.dialect)
        else:
            ...
            expressions = apply_index_offset(this, expressions, -self.dialect.INDEX_OFFSET, dialect=self.dialect)
            this = self.expression(
                exp.Bracket,
                this=this,
                expressions=expressions,
    ...
    def _parse_if(self) -> t.Optional[exp.Expression]:
        if self._match(TokenType.L_PAREN):
            args = self._parse_csv(lambda: self._parse_alias(self._parse_assignment(), explicit=True))
    ...
    def _parse_string_agg(self) -> exp.GroupConcat:
        if self._match(TokenType.DISTINCT):
            args: t.List[t.Optional[exp.Expression]] = [self.expression(exp.Distinct, expressions=[self._parse_assignment()])]
            if self._match(TokenType.COMMA):
        ...
        if self._match_text_seq("ON", "OVERFLOW"):
            ...
            if self._match_text_seq("ERROR"):
                ...
                on_overflow = self.expression(
                    ...
                    this=self._parse_string(),
                    with_count=(self._match_text_seq("WITH", "COUNT") or not self._match_text_seq("WITHOUT", "COUNT")),
                )
        else:
            on_overflow = None
    ...
    def _parse_convert(self, strict: bool, safe: t.Optional[bool] = None) -> t.Optional[exp.Expression]:
        ...
        if self._match(TokenType.USING):
            to: t.Optional[exp.Expression] = self.expression(exp.CharacterSet, this=self._parse_var())
    ...
    def _parse_on_handling(self, on: str, *values: str) -> t.Optional[str] | t.Optional[exp.Expression]:
    ...
    def _parse_json_object(self, agg=False):
        ...
        expressions = [star] if star else self._parse_csv(lambda: self._parse_format_json(self._parse_json_key_value()))
        ...
        return_type = self._match_text_seq("RETURNING") and self._parse_format_json(self._parse_type())
    ...
    def _parse_position(self, haystack_first: bool = False) -> exp.StrPosition:
        ...
        if self._match(TokenType.IN):
            return self.expression(exp.StrPosition, this=self._parse_bitwise(), substr=seq_get(args, 0))
        ...
        return self.expression(exp.StrPosition, this=haystack, substr=needle, position=seq_get(args, 2))
    ...
    def _parse_window(self, this: t.Optional[exp.Expression], alias: bool = False) -> t.Optional[exp.Expression]:
        ...
        if kind:
            ...
            exclude = self._parse_var_from_options(self.WINDOW_EXCLUDE_OPTIONS) if self._match_text_seq("EXCLUDE") else None
    ...
    def _parse_window_spec(self) -> t.Dict[str, t.Optional[str | exp.Expression]]:
        ...
        return {
            "value": ((self._match_text_seq("UNBOUNDED") and "UNBOUNDED") or (self._match_text_seq("CURRENT", "ROW") and "CURRENT ROW") or self._parse_bitwise()),
    ...
    def _parse_id_var(
        ...
        if not expression and ((any_token and self._advance_any()) or self._match_set(tokens or self.ID_VAR_TOKENS)):
    ...
    def _parse_tokens(self, parse_method: t.Callable, expressions: t.Dict) -> t.Optional[exp.Expression]:
        this = parse_method()
    ...
    def _parse_select_or_expression(self, alias: bool = False) -> t.Optional[exp.Expression]:
        return self._parse_select() or self._parse_set_operations(self._parse_alias(self._parse_assignment(), explicit=True) if alias else self._parse_assignment())

    def _parse_ddl_select(self) -> t.Optional[exp.Expression]:
        return self._parse_query_modifiers(self._parse_set_operations(self._parse_select(nested=True, parse_subquery_alias=False)))
    ...
    def _parse_add_column(self) -> t.Optional[exp.ColumnDef]:
        ...
        if self._match_texts(("FIRST", "AFTER")):
            ...
            column_position = self.expression(exp.ColumnPosition, this=self._parse_column(), position=position)
    ...
    def _parse_alter_table_add(self) -> t.List[exp.Expression]:
        def _parse_add_alteration() -> t.Optional[exp.Expression]:
            ...
            if self._match_set(self.ADD_CONSTRAINT_TOKENS, advance=False):
                return self.expression(exp.AddConstraint, expressions=self._parse_csv(self._parse_constraint))
            ...
            if self._match_pair(TokenType.PARTITION, TokenType.L_PAREN, advance=False):
                return self.expression(
                    ...
                    exists=exists,
                    this=self._parse_field(any_token=True),
                    location=self._match_text_seq("LOCATION", advance=False) and self._parse_property(),
                )

            return None

        if not self._match_set(self.ADD_CONSTRAINT_TOKENS, advance=False) and (not self.dialect.ALTER_TABLE_ADD_REQUIRED_FOR_EACH_COLUMN or self._match_text_seq("COLUMNS")):
            schema = self._parse_schema()

            return ensure_list(schema) if schema else self._parse_csv(self._parse_column_def_with_exists)
    ...
    def _parse_alter_sortkey(self, compound: t.Optional[bool] = None) -> exp.AlterSortKey:
        ...
        if self._match(TokenType.L_PAREN, advance=False):
            return self.expression(exp.AlterSortKey, expressions=self._parse_wrapped_id_vars(), compound=compound)

        self._match_texts(("AUTO", "NONE"))
        return self.expression(exp.AlterSortKey, this=exp.var(self._prev.text.upper()), compound=compound)
    ...
    def _parse_alter_table_set(self) -> exp.AlterSet:
        ...
        if self._match(TokenType.L_PAREN, advance=False) or self._match_text_seq("TABLE", "PROPERTIES"):
    ...
    def _parse_analyze(self) -> exp.Analyze | exp.Command:
        ...
        if self._match_text_seq("WITH", "SYNC", "MODE") or self._match_text_seq("WITH", "ASYNC", "MODE"):
    ...
    def _parse_analyze_statistics(self) -> exp.AnalyzeStatistics:
        ...
        return self.expression(exp.AnalyzeStatistics, kind=kind, option=option, this=this, expressions=expressions)
    ...
    def _parse_analyze_validate(self) -> exp.AnalyzeValidate:
        ...
        if self._match_text_seq("REF", "UPDATE"):
            ...
            if self._match_text_seq("CASCADE", "FAST"):
                this = "CASCADE FAST"
            elif self._match_text_seq("CASCADE", "COMPLETE") and self._match_texts(("ONLINE", "OFFLINE")):
    ...
    def _parse_analyze_histogram(self) -> exp.AnalyzeHistogram:
        ...
        if self._match_text_seq("HISTOGRAM", "ON"):
            ...
            if self._match_texts(("MANUAL", "AUTO")) and self._match(TokenType.UPDATE, advance=False):
            ...
            if self._match(TokenType.INSERT):
                ...
                if this:
                    ...
                    then = self.expression(
                        exp.Insert,
                        this=exp.var("ROW") if self._match_text_seq("ROW") else self._parse_value(values=False),
    ...
    def _parse_set_transaction(self, global_: bool = False) -> exp.Expression:
        ...
        characteristics = self._parse_csv(lambda: self._parse_var_from_options(self.TRANSACTION_CHARACTERISTICS))
        return self.expression(
            exp.SetItem,
    ...
    def _parse_set(self, unset: bool = False, tag: bool = False) -> exp.Set | exp.Command:
        index = self._index
        set_ = self.expression(exp.Set, expressions=self._parse_csv(self._parse_set_item), unset=unset, tag=tag)
    ...
    def _match_texts(self, texts, advance=True):
        if self._curr and self._curr.token_type != TokenType.STRING and self._curr.text.upper() in texts:
            if advance:
                self._advance()
            return True
        return None
    ...
    def _match_text_seq(self, *texts, advance=True):
        index = self._index
        for text in texts:
            if self._curr and self._curr.token_type != TokenType.STRING and self._curr.text.upper() == text:
                self._advance()
            else:
                self._retreat(index)
                return None
    ...
    def _replace_lambda(self, node: t.Optional[exp.Expression], expressions: t.List[exp.Expression]) -> t.Optional[exp.Expression]:
        if not node:
            return node
    ...
    def _parse_copy_parameters(self) -> t.List[exp.CopyParameter]:
        ...
        while self._curr and not self._match(TokenType.R_PAREN, advance=False):
            ...
            if prev in self.COPY_INTO_VARLEN_OPTIONS and self._match(TokenType.L_PAREN, advance=False):
    ...
    def _parse_credentials(self) -> t.Optional[exp.Credentials]:
        ...
        if self._match_text_seq("CREDENTIALS"):
            ...
            creds = self._parse_wrapped_options() if self._match(TokenType.EQ) else self._parse_field()
    ...
    def _parse_copy(self) -> exp.Copy | exp.Command:
        ...
        this = self._parse_select(nested=True, parse_subquery_alias=False) if self._match(TokenType.L_PAREN, advance=False) else self._parse_table(schema=True)
    ...
    def _parse_ceil_floor(self, expr_type: t.Type[TCeilFloor]) -> TCeilFloor:
        ...
        return expr_type(this=this, decimals=decimals, to=self._match_text_seq("TO") and self._parse_var())
    ...
    def _parse_grant_privilege(self) -> t.Optional[exp.GrantPrivilege]:
        ...
        expressions = self._parse_wrapped_csv(self._parse_column) if self._match(TokenType.L_PAREN, advance=False) else None
    ...
    def _parse_max_min_by(self, expr_type: t.Type[exp.AggFunc]) -> exp.AggFunc:
        ...
        return self.expression(expr_type, this=seq_get(args, 0), expression=seq_get(args, 1), count=seq_get(args, 2))

    def _identifier_expression(self, token: t.Optional[Token] = None, **kwargs: t.Any) -> exp.Identifier:
    ...
    def _parse_pipe_syntax_select(self, query: exp.Select) -> exp.Select:
        ...
        return self._build_pipe_cte(query=query.select(*select.expressions, append=False), expressions=[exp.Star()])
    ...
    def _parse_pipe_syntax_aggregate_group_order_by(self, query: exp.Select, group_by_exists: bool = True) -> exp.Select:
    ...
    def _parse_pipe_syntax_aggregate(self, query: exp.Select) -> exp.Select:
        ...
        if self._match(TokenType.GROUP_BY) or (self._match_text_seq("GROUP", "AND") and self._match(TokenType.ORDER_BY)):
